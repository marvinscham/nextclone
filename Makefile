...
build-linux:
	mkdir -p $(DIST)
	CGO_ENABLED=1 GOOS=linux GOARCH=amd64 go build $(GO_BUILD_FLAGS) -ldflags "$(GO_LDFLAGS)" -o $(DIST)/$(APP)-linux-amd64 $(CMD)

build-windows:
	mkdir -p $(DIST)
	CGO_ENABLED=1 GOOS=windows GOARCH=amd64 CC=x86_64-w64-mingw32-gcc go build $(GO_BUILD_FLAGS) -ldflags "$(GO_LDFLAGS) -H windowsgui" -o $(DIST)/$(APP)-windows-amd64.exe $(CMD)
